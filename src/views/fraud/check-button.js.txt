...
function delayDisableButton(button, visualStatus) {
  ...
  var changeUserMessage = setTimeout(function () {
    var screenReaderStatus = document.getElementById("screen-reader-status");
    visualStatus.textContent =
      '{{ translate("buttons.nextStillWaitingVisible") | safe }}';
    screenReaderStatus.textContent =
      '{{ translate("buttons.nextStillWaitingScreenReader") | safe }}';
  }, longWait);
}
