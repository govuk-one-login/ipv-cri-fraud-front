...
|-------------------------|----------------------------------------------------|------------------------------------------------------------------------------------------------------------|
| CREDENTIAL_ISSUER_LABEL | Fraud CRI Dev                                    | There might be many different buttons on the credential issuer page, this allows selection of a single one |
...
```
  @mock-api:fraud-error
  Scenario: Fraud error
  ...
```
This scenario will be configured to send a `scenario-id` header of `fraud-error` on every web browser request.
